...
import { HighlighterPipe } from '../src/app/project-search/providers/highlighter.pipe';

describe('Project search page component tests - bigger viewport', () => {
  beforeEach(() => {
    Mount(800,800);
  })
...
function Mount(width: number, height: number) {
  ...
  mount(ProjectSearchPageComponent, {
    ...
    declarations: [AddProjectReportComponent, HighlighterPipe]
  });
}
...
function CheckOneFilter(order: number) {
...
function CheckElementsAfterSearch() {
  ...
  cy.get('mat-icon#up_button').should('contain', 'arrow_upward').click().then(() => {
    cy.get('mat-icon#up_utton').should('not.exist');
  });
